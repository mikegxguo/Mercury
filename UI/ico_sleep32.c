/*********************************************************************
*                SEGGER Microcontroller GmbH & Co. KG                *
*        Solutions for real time microcontroller applications        *
*                           www.segger.com                           *
**********************************************************************
*                                                                    *
* C-file generated by                                                *
*                                                                    *
*        µC/GUI-BitmapConvert V5.08.                                 *
*        Compiled Jan 13 2011, 12:35:22                              *
*        (c) 2002-2010  Micrium, Inc.                                *
*        www.micrium.com                                             *
*                                                                    *
*        (c) 1998-2010  Segger Microcontroller GmbH & Co. KG *                                                                    *
**********************************************************************
*                                                                    *
* Source file: ico_sleep32                                           *
* Dimensions:  32 * 32                                               *
* NumColors:   2                                                     *
*                                                                    *
**********************************************************************
*/

#include <stdlib.h>

#include "GUI.h"

#ifndef GUI_CONST_STORAGE
  #define GUI_CONST_STORAGE const
#endif

/*   Palette
The following are the entries of the palette table.
Every entry is a 32-bit value (of which 24 bits are actually used)
the lower   8 bits represent the Red component,
the middle  8 bits represent the Green component,
the highest 8 bits (of the 24 bits used) represent the Blue component
as follows:   0xBBGGRR
*/

static GUI_CONST_STORAGE GUI_COLOR Colorsico_sleep32[] = {
     0x000000,0xFFFFFF
};

static GUI_CONST_STORAGE GUI_LOGPALETTE Palico_sleep32 = {
  2,        /* number of entries */
  0,         /* No transparency */
  &Colorsico_sleep32[0]
};

static GUI_CONST_STORAGE unsigned char acico_sleep32[] = {
  ________, ________, ________, ________,
  ________, ________, ________, ________,
  ________, ________, ________, ________,
  ________, ________, ________, ________,
  ________, ______XX, XXXX____, ________,
  ________, ___XXXXX, XXXXX___, ________,
  ________, __XXXXXX, XXXXX___, ________,
  ________, XXXXXXXX, XXXX____, ________,
  ________, XXXXXXXX, XX______, ________,
  _______X, XXXXXXXX, X_______, ________,
  ______XX, XXXXXXXX, X_______, ________,
  ______XX, XXXXXXXX, ________, ________,
  _____XXX, XXXXXXXX, ________, ________,
  _____XXX, XXXXXXXX, ________, ________,
  _____XXX, XXXXXXXX, ________, ________,
  ____XXXX, XXXXXXXX, ________, ________,
  ____XXXX, XXXXXXXX, ________, ________,
  ____XXXX, XXXXXXXX, ________, ________,
  ____XXXX, XXXXXXXX, X_______, ________,
  _____XXX, XXXXXXXX, XX______, ________,
  _____XXX, XXXXXXXX, XXX_____, __XX____,
  _____XXX, XXXXXXXX, XXXX____, _XXXX___,
  ______XX, XXXXXXXX, XXXXXXXX, XXXXX___,
  ______XX, XXXXXXXX, XXXXXXXX, XXXXX___,
  _______X, XXXXXXXX, XXXXXXXX, XXXX____,
  ________, XXXXXXXX, XXXXXXXX, XXX_____,
  ________, _XXXXXXX, XXXXXXXX, XX______,
  ________, __XXXXXX, XXXXXXXX, ________,
  ________, ____XXXX, XXXXXXX_, ________,
  ________, _______X, XXXX____, ________,
  ________, ________, ________, ________,
  ________, ________, ________, ________
};

GUI_CONST_STORAGE GUI_BITMAP bmico_sleep32 = {
  32, /* XSize */
  32, /* YSize */
  4, /* BytesPerLine */
  1, /* BitsPerPixel */
  acico_sleep32,  /* Pointer to picture data (indices) */
  &Palico_sleep32  /* Pointer to palette */
};

/* *** End of file *** */
